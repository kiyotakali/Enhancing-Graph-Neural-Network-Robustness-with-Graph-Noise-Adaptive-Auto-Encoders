...
class NRGNN_subgraph_cleaner:
    ...
    def train(self, epoch, features, edge_index, idx_train, idx_val):
        ...
        for i in range(len(nodes)):
            ...
            # edges_tensor = torch.tensor(edges).t().contiguous()
            edges_tensor = torch.tensor(edges, dtype=torch.long).t().contiguous() # 修改了这一行
            x0 = representations[edges_tensor[0]]
            x1 = representations[edges_tensor[1]]
            output = torch.sum(torch.mul(x0, x1), dim=1)
            val,indices = torch.topk(output,50,largest=True)
            indices = indices.tolist()
            selected_values = [idx_train[i] for i in indices]
            select_label = new_label[selected_values]
            mode_value, mode_count = torch.mode(select_label)
            if(mode_count>48):
                # print(nodes[i])
                new_label[nodes[i]] = mode_value
        mask_train = []
        for i in range(len(idx_mask)):
            edges = []  # 存储边的列表
            for j in range(len(idx_mask)):
                edges.append((mask_nodes[i], mask_nodes[j]))
            edges_tensor = torch.tensor(edges, dtype=torch.long).t().contiguous() # 修改了这一行
            x0 = representations[edges_tensor[0]]
            x1 = representations[edges_tensor[1]]
            output = torch.sum(torch.mul(x0, x1), dim=1)
            val,indices = torch.topk(output,30,largest=True)
            indices = indices.tolist()
            selected_values = [idx_mask[i] for i in indices]
            select_label = self.labels[selected_values]
            mode_value, mode_count = torch.mode(select_label)
            if(mode_count>28):
                mask_train.append(idx_mask[i])
                print(idx_mask[i],mode_value,self.labels[idx_mask[i]])
                new_label[idx_mask[i]] = mode_value
        mask_train = torch.from_numpy(np.array(mask_train))
        idx_train = torch.from_numpy(idx_train)
        new_idx_train = torch.cat([idx_train, mask_train], dim=0)
        # print('new_idx_train',new_idx_train.shape)
        new_idx_train = new_idx_train.long()
        # print('new_idx_train',new_idx_train.shape)
        # print(torch.cuda.memory_summary())
        # self.labels = torch.cat([self.labels, new_label], dim=0)
        new_label= torch.cat([self.labels, new_label], dim=0)
