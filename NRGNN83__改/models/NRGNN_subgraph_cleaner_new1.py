...
class NRGNN_subgraph_cleaner_new1:
    ...
    def fit(self, features, adj, labels, idx_train, idx_val):
        ...
        models = GCN1(features.size(1), labels.max().item() + 1) # 实例化模型
        optimizer = torch.optim.Adam(models.parameters(), lr=0.01, weight_decay=5e-4) # 优化器
        models.train() # 训练模式
        for epoch in range(200): # 迭代次数
            optimizer.zero_grad() # 梯度清零
            representations, rec_loss = self.estimator(edge_index,features)
            edge_weight = self.estimator.get_estimated_weigths(self.pred_edge_index,representations)
            out = models(self,labels,edge_index,edge_weight) # 前向传播
            loss = F.nll_loss(out[idx_train], labels[idx_train]) # 计算损失
            loss.backward() # 反向传播
            optimizer.step() # 更新参数
